...
def _localizar_planilha() -> Path:
    ...
    raise FileNotFoundError(f"Planilha de usuários não encontrada. Nomes aceitos: {nomes}")


@st.cache_data(ttl=300, show_spinner=False)
def _carregar_usuarios(caminho: str, assinatura: tuple[int, int]) -> dict[str, str]:
    del assinatura
...
def _usuarios_cadastrados() -> dict[str, str]:
    ...
    status = caminho.stat()
    return _carregar_usuarios(str(caminho), (status.st_mtime_ns, status.st_size))
...
def exigir_login() -> dict[str, str]:
    ...
    if not acesso_sessao:
        ...
        with st.form("form_login", clear_on_submit=False):
            login_digitado = st.text_input("Usuário", placeholder="Ex.: Suene.silva")
    ...
    with st.sidebar:
        ...
        if st.button("Sair", key="sair_do_painel", width="stretch"):
            ...
            st.rerun()
        st.markdown("---")
    return {"LOGIN": login_sessao, "ACESSO": acesso_sessao}
...
def filtrar_por_acesso(
    ...
    if acesso_n == "RIOF":
        permitido = regional.str.contains("RIO VERDE", na=False) | regional.str.startswith("RIOF")
    elif acesso_n == "MORF":
        permitido = regional.str.contains("MORRINHOS", na=False) | regional.str.startswith("MORF")
    else:
