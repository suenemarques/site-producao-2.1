...
def carregar():
    ...
    d["DT_CONCLUSAO"] = pd.to_datetime(d["DT_CONCLUSAO"], errors="coerce")
...
with st.sidebar:
    ...
    for regional in regs:
        if regional in VALOR_EQUIPE:
            faltas[regional] = st.number_input(f"Faltas por mês — {regional.title()}",min_value=0,value=0,step=1,help=f"Cada falta reduz {moeda(VALOR_EQUIPE[regional])} do faturamento mensal.")

d = d0[d0.REGIONAL.isin(regs)&d0.MES_REF.isin(meses)&d0.EQUIPE.isin(equipes)].copy()
tot = d.groupby("CLASSIFICACAO").VALOR.sum()
odi = float(tot.get("ODI",0)); odd = float(tot.get("ODD",0)); capex = odi+odd
...
st.caption(f"Período pela DT_CONCLUSAO · CAPEX = ODI + ODD · Dados atualizados em: {atualizado:%d/%m/%Y às %H:%M}" if pd.notna(atualizado) else "Período pela DT_CONCLUSAO · CAPEX = ODI + ODD")
c1,c2,c3,c4 = st.columns(4)
c1.metric("Valor do contrato",moeda(valor_contrato)); c2.metric("Desconto por faltas",moeda(desconto_faltas)); c3.metric("Valor a faturar",moeda(valor_faturar)); c4.metric("Capitalização",f"{capitalizacao:.2f}%".replace(".",","))
c1,c2,c3,c4 = st.columns(4)
c1.metric("CAPEX",moeda(capex)); c2.metric("Clandestino no CAPEX",moeda(clandestino)); c3.metric("ODI",moeda(odi)); c4.metric("ODD",moeda(odd))
p1,p2,p3 = st.columns(3)
p1.metric("OPEX calculado",moeda(opex)); p2.metric("% OPEX",f"{100-capitalizacao:.2f}%".replace(".",",") if valor_faturar else "0,00%"); p3.metric("Lançamentos da base",f"{len(d):,}".replace(",","."))
...
with a:
    ...
    fig = px.bar(comp,x="Classificação",y="Valor",color="Classificação",text="Rótulo",title="Composição dos custos",color_discrete_map={"ODI":"#38BDF8","ODD":"#A78BFA","OPEX":"#F59E0B"})
...
with b:
    ...
    for mes in sorted(set(meses)):
        dm = d[d.MES_REF.eq(mes)]; tm = dm.groupby("CLASSIFICACAO").VALOR.sum()
    ...
    fig = px.bar(mensal,x="Mês",y="VALOR",color="CLASSIFICACAO",barmode="stack",text="Rótulo",title="Evolução mensal")
...
with a:
    ...
    fig = px.bar(serv,x="Valor",y="TIPO_SERVICO",orientation="h",color="CLASSIFICACAO",text="Quantidade",title="Serviços que compõem o CAPEX")
    fig.update_traces(textposition="auto",cliponaxis=False)
    st.plotly_chart(tema(fig),width="stretch")
with b:
    eq = d[d.CLASSIFICACAO.isin(["ODI","ODD"])].groupby(["EQUIPE","CLASSIFICACAO"],as_index=False).VALOR.sum()
    fig = px.bar(eq,x="EQUIPE",y="VALOR",color="CLASSIFICACAO",barmode="stack",title="CAPEX por equipe")
    fig.update_traces(textposition="auto",cliponaxis=False)
    st.plotly_chart(tema(fig),width="stretch")
...
st.dataframe(d.sort_values("DT_CONCLUSAO",ascending=False),width="stretch",hide_index=True)
