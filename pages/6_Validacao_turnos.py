...
from auth_site import exigir_acesso_geral, exigir_login


st.set_page_config(page_title="Validação de Turnos", page_icon="🕒", layout="wide")

# Bloqueia RIOF e MORF antes de ler ou exibir qualquer dado desta página.
usuario = exigir_login()
exigir_acesso_geral(usuario)

BASE = Path(__file__).resolve().parents[1]
ARQ = BASE / "dados" / "validacao_turnos.parquet"
METAS = BASE / "METAS 2026.xlsx"
...
if not ARQ.is_file():
    st.error("Base de turnos não encontrada.")
...
def carregar():
    ...
    for c in ["DATA_TURNO","DT_BAIXA","HOST_VI_DT_INI_DESLOCAMENTO","HOST_VI_DT_FIM_DESLOCAMENTO","HOST_VI_DT_INI_SERVICO","HOST_VI_DT_FIM_SERVICO"]:
        d[c] = pd.to_datetime(d[c],errors="coerce")
    return d


d0 = carregar()
meses_disp = sorted(d0.DATA_TURNO.dt.month.dropna().astype(int).unique())

with st.sidebar:
    st.markdown("### 🕒 Validação de Turnos")
    st.caption("Acesso exclusivo para usuários GERAL")
    ...
    st.button("🕒 Validação de Turnos",disabled=True,width="stretch")
    st.markdown("---")
    ...
    inicio_padrao = pd.Timestamp(2026,mes,1).date()
    fim_padrao = pd.Timestamp(2026,mes,monthrange(2026,mes)[1]).date()
    ...
    equipes_disp = sorted(d0[d0.REGIONAL_TURNO.isin(regs)].PRX_DESCRICAO.unique())
    equipes = st.multiselect("Equipe",equipes_disp,default=equipes_disp)

ini = pd.Timestamp(periodo[0] if isinstance(periodo,(tuple,list)) else periodo)
fim = pd.Timestamp(periodo[-1] if isinstance(periodo,(tuple,list)) else periodo)+pd.Timedelta(days=1)-pd.Timedelta(microseconds=1)
...
resumo = diario.groupby(["REGIONAL_TURNO","PRX_DESCRICAO"],as_index=False).agg(Dias=("DATA_TURNO","nunique"),Horas=("HORAS","sum"),SS=("SS","sum"))
resumo["Meta horas"] = resumo.Dias*8
resumo["Média diária"] = resumo.Horas/resumo.Dias.replace(0,pd.NA)
resumo["Situação"] = resumo.apply(lambda r:"Dentro do esperado" if r.Horas>=r["Meta horas"] else "Abaixo do esperado",axis=1)
atualizado = pd.to_datetime(d0.get("ATUALIZADO_EM",pd.Series(dtype=str)),errors="coerce").max()

st.title("Validação de Turnos")
st.caption(f"{ini:%d/%m/%Y} a {fim:%d/%m/%Y} · primeiro início ao último fim por equipe/dia · Dados atualizados em: {atualizado:%d/%m/%Y às %H:%M}" if pd.notna(atualizado) else f"{ini:%d/%m/%Y} a {fim:%d/%m/%Y}")
c1,c2,c3,c4 = st.columns(4)
horas = float(resumo.Horas.sum()); meta = float(resumo["Meta horas"].sum())
c1.metric("Horas realizadas",f"{horas:,.1f} h".replace(",","X").replace(".",",").replace("X","."))
c2.metric("Meta de horas",f"{meta:,.1f} h".replace(",","X").replace(".",",").replace("X","."))
c3.metric("Dias-equipe",inteiro(resumo.Dias.sum()))
c4.metric("SS executadas",inteiro(d.NR_OS.nunique()))

comp = resumo.melt(id_vars=["PRX_DESCRICAO","Situação"],value_vars=["Meta horas","Horas"],var_name="Tipo",value_name="Quantidade")
fig = px.bar(comp,x="PRX_DESCRICAO",y="Quantidade",color="Tipo",barmode="group",text_auto=".1f",title="Meta x realizado de horas por equipe",category_orders={"Tipo":["Meta horas","Horas"]},color_discrete_map={"Meta horas":"#64748B","Horas":"#38BDF8"})
fig.update_traces(textposition="auto",cliponaxis=False)
st.plotly_chart(tema(fig,500),width="stretch")

a,b = st.columns(2)
with a:
    fig = px.bar(resumo,x="PRX_DESCRICAO",y="Média diária",color="Situação",text_auto=".1f",title="Média diária por equipe",color_discrete_map={"Dentro do esperado":"#34D399","Abaixo do esperado":"#F87171"})
    fig.add_hline(y=8,line_dash="dash",line_color="#F59E0B",annotation_text="Meta 8h")
    fig.update_traces(textposition="auto",cliponaxis=False)
    st.plotly_chart(tema(fig),width="stretch")
with b:
    linha = diario.groupby("DATA_TURNO",as_index=False).HORAS.sum()
    fig = px.line(linha,x="DATA_TURNO",y="HORAS",markers=True,text="HORAS",title="Horas trabalhadas por dia")
    fig.update_traces(texttemplate="%{text:.1f}",textposition="top center")
    st.plotly_chart(tema(fig),width="stretch")

st.subheader("Resumo por equipe")
st.dataframe(resumo,width="stretch",hide_index=True)
st.subheader("Base por SS")
st.dataframe(d.sort_values(["DATA_TURNO","PRX_DESCRICAO"]),width="stretch",hide_index=True)
